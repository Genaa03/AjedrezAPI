...
import com.example.tpajedrezapi.dtos.common.ErrorApi;
...
public class ControllerExceptionHandler {
    ...
    public ResponseEntity<ErrorApi> handleError(IllegalArgumentException exception) {
        ErrorApi errorApi = new ErrorApi(LocalDateTime.now().toString(),400,exception.getLocalizedMessage(), exception.getMessage());
    ...
    public ResponseEntity<ErrorApi> handleError(ResponseStatusException exception) {
        ErrorApi error = buildError(exception.getReason(), HttpStatus.valueOf(exception.getRawStatusCode()));
        return ResponseEntity.status(exception.getRawStatusCode()).body(error);
    }
